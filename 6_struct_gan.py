...
def main(cfg):
    ...
    gen_opt = AdamW(0.001, 1.0, beta_1=0.5, clipnorm=cfg.clipnorm)
    disc_opt = tf.optimizers.SGD(1.0, clipnorm=cfg.clipnorm)
    smooth_opt = AdamW(0.001, 1.0, beta_1=0.5, clipnorm=cfg.clipnorm)
    ...
    def gen_train_step(
        ...
        with tf.GradientTape() as tape:
            g_noise = generator(logits_y, noise, training=True)
        ...
        # intervent on noise to signal ratio
            ...
            # fake = logits_y + tf.expand_dims(scale, -1) * g_noise
            fake = logits_y + g_noise

            w = tf.constant(1.0)
            with tf.GradientTape() as dummy_tape:
                dummy_tape.watch(w)
                disc = w * discriminator(fake, training=True)
                smo = w * smoother(fake, training=True)
                smo = denoiser.masked_softmax(smo, mask)

                gen_loss = - tf.math.log(tf.sigmoid(disc) + 1e-9)
                smo_loss = js(smo, y)
                ...
                w_loss = gen_loss + cycle_reg * smo_loss + norm_loss
        ...
        return gen_loss, smo_loss, fake, disc, smo, norm_loss

    @tf.function
    def disc_train_step(logits_x, fake, noise, reg_wt, train=True):
        with tf.GradientTape() as tape:
            w = tf.constant(1.0)
            with tf.GradientTape() as dummy_tape:
                dummy_tape.watch(w)
                disc_fake = w * discriminator(fake, training=True)
                disc_real = w * discriminator(logits_x, training=True)
                disc_fake = tf.math.log(1.0 - tf.sigmoid(disc_fake) + 1e-9)
                disc_real = tf.math.log(tf.sigmoid(disc_real) + 1e-9)
    ...
    @tf.function
    def smoother_train_step(fake, y, mask, reg_wt, envs=1):
        with tf.GradientTape() as tape:
            w = tf.constant(1.0)
            smo = smoother(fake, training=True)
    ...
    for e in range(epochs):
        ...
        for j, batch_data in enumerate(
            ...
            train_gen = j % cfg.train_smoother_every == 0

            # update_gen = j % cfg.train_gen_every == 0
            gen_loss, cycle_loss, fake, gen_disc, smo, norm_loss = gen_train_step(
                logits_y, noise, alpha, y, mask, reg_wt,
                cr, expl, max_nr, train_gen
            )
            ...
            disc_loss, disc_fake, disc_real = disc_train_step(
                logits_x, fake, noise, reg_wt, train_disc
            )
        ...
        for batch_data in batches(ood_data, ood_data, 4 * batch_size):
            logits_y, logits_x, noise, alpha, x, y, mask, _ = batch_data
