...
class Replay:
    ...
    def sample(self, blocks) -> np.ndarray:
        ...
        ix_b = (ix_b * self._block * ix_b) % self._N
        ix = []
        for i in ix_b:
            for j in range(self._block):
                ix.append(i + j)
        return self._storage[ix]

    def add(self, block: np.ndarray) -> None:
        ix = [
            (self._next_idx + i) % self._N
            for i in range(block.shape[0])
        ]
        self._storage[ix] = block
        self._next_idx += block.shape[0]
...
def main(cfg):
    ...
    def gen_train_step(
        ...
        with tf.GradientTape() as tape:
            ...
            noise = mask * nadd * C
            logits_fake = tf.clip_by_value(logits_y + noise, -C, C)
    ...
    @tf.function
    def disc_train_step(logits_x, logits_fake, reg_wt, train=True):
    ...
    for e in range(epochs):
        ...
        for j, batch_data in enumerate(
            ...
            if cfg.replay_blocks > 0:
                buffer.add(logits_fake[:(cfg.replay_blocks * cfg.env_block_size)].numpy())
            else:
                buffer.add(logits_fake.numpy())
            ...
            logits_replay = tf.constant(buffer.sample(blocks))
            logits_fake_mix = [logits_replay, logits_fake[:(cfg.batch_size // 2)]]
            logits_fake_mix = tf.concat(logits_fake_mix, 0)
        ...
        if e + 1 % cfg.ckpt_every == 0:
            generator.save_weights(f"gen_{e + 1}.h5")
            discriminator.save_weights(f"disc_{e + 1}.h5")
